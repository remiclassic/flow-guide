...
type CourseLibraryRow = PublishedCourseListItem & {
  moduleCount: number;
  lessonCount: number;
  estMinutes: number;
  progress: {
  ...
  statusLabel: string;
  isScheduledTeaser: boolean;
  courseHasAccess: boolean;
};

export default async function CoursesLibraryPage({ params }: PageProps) {
  ...
  const items: CourseLibraryRow[] = await Promise.all(
    rows.map(async (course): Promise<CourseLibraryRow> => {
  ...
  return (
    <section className="flex-1 bg-[radial-gradient(circle_at_top_left,hsl(var(--primary)/0.08),transparent_34rem),linear-gradient(180deg,hsl(var(--background)),hsl(var(--muted)/0.28))] p-3 pb-10 sm:p-4 lg:p-6">
      ...
      <div className="rounded-[2rem] border border-white/80 bg-white/90 p-7 shadow-[0_22px_74px_-46px_hsl(var(--primary)/0.5)] sm:p-8 lg:p-10">
        ...
        >
          {items.length === 1
            ? t('home.programsSingular', { count: items.length })
            : t('home.programsPlural', { count: items.length })}
        </Badge>
      </div>
      </div>
              ...
              <CardContent className="space-y-7 pb-8">
                <div className="grid grid-cols-3 gap-2 text-xs text-muted-foreground">
                  <span className="rounded-xl bg-muted/35 px-3 py-2">
                    <span className="block text-muted-foreground/75">{t('shared.modules')}</span>
                    <span className="text-sm font-semibold text-foreground">{course.moduleCount}</span>
                  </span>
                  <span className="rounded-xl bg-muted/35 px-3 py-2">
                    <span className="block text-muted-foreground/75">{t('shared.lessons')}</span>
