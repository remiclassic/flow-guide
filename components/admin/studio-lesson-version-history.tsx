...
import {
  createLessonVersionSnapshotAction,
  getLessonVersionDetailAction,
  listLessonVersionsAction,
  restoreLessonVersionAction,
  type LessonVersionListItem,
...
export function StudioLessonVersionHistory({
  courseSlug,
  lessonKey,
  onRestored,
}: {
  ...
  const [versions, setVersions] = useState<LessonVersionListItem[] | null>(
    null
  );
  ...
  const refresh = useCallback(() => {
    startTransition(async () => {
      setLoadError(null);
      const res = await listLessonVersionsAction(courseSlug, lessonKey);
      if (!res.ok) {
  ...
  async function createSnapshot() {
    startTransition(async () => {
      ...
      const res = await createLessonVersionSnapshotAction(fd);
      if (res.ok) {
        setSnapshotNote('');
        refresh();
      }
      ...
      {open ? (
        <div className="space-y-4 border-t border-stone-200/80 px-5 pb-5 pt-4">
          <p className="text-xs leading-relaxed text-stone-600">
            Snapshots capture draft lesson content and metadata. Restoring
            never changes what learners already see — it only replaces your
            working draft. History is never deleted.
          </p>

          <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            <div className="min-w-0 flex-1 space-y-1.5">
              <Label htmlFor="snap-note" className="text-xs text-stone-600">
              ...
              <Input
                ...
                placeholder="Before restructure…"
          ...
          <div className="max-h-64 space-y-2 overflow-y-auto pr-1">
            {versions === null ? (
              ...
              <p className="text-sm text-stone-500">
                No snapshots yet. Save work with the button above or publish —
                a snapshot is added automatically before each publish.
              </p>
            ) : (
              versions.map((v) => (
                <div
...
function RestoreLessonVersionModal({
  courseSlug,
  lessonKey,
  versionId,
  backupFirst,
  ...
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      const res = await getLessonVersionDetailAction(
        courseSlug,
        lessonKey,
        versionId
      );
      ...
      setDetail(
        `Titles: ${s.titleEn} / ${s.titleEs}. Draft blocks: ${s.draftBodyBlocks?.length ?? 0} blocks.`
      );
    })();
    return () => {
      cancelled = true;
    };
  }, [courseSlug, lessonKey, versionId]);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="restore-lesson-title"
    >
      <button
        type="button"
      ...
      >
        <h2
          id="restore-lesson-title"
          className="text-base font-semibold text-stone-950"
        >
          Restore this version to your draft?
        </h2>
        <p className="text-sm leading-relaxed text-stone-600">
          Your published lesson stays as-is for learners. We&apos;ll replace
          only draft content, quiz, and lesson metadata with this snapshot, and
        ...
        <label className="flex cursor-pointer items-start gap-2 text-sm text-stone-800">
          ...
          />
          <span>
            Save a backup snapshot of the current draft before restoring
            (recommended).
          </span>
        </label>
          ...
          <Button
            ...
            onClick={() => {
              startTransition(async () => {
                ...
                const res = await restoreLessonVersionAction(fd);
                if (res.ok) onDone();
              });
            }}
          >
