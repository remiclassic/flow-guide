...
export type CourseOverviewOutlineModule = {
  id: number;
  titleEn: string;
  titleEs: string | null;
  lessons: Array<{
...
export type CourseOverviewExperienceProps = {
  courseTitle: string;
  overviewBlurb: string | null;
  unlocked: boolean;
  outline: CourseOverviewOutlineModule[];
  ratio: { percent: number; completed: number; total: number };
  modRows: ModuleProgressRow[];
  completedSet: ReadonlySet<number>;
  nextLesson: NextLessonTarget | null;
  continueHref: string;
  cta: string;
  lessonHref: (lessonKey: string) => string;
  /** Optional banner above hero (e.g. admin preview notice). */
  banner?: ReactNode;
  /** When set, shows an extra “Draft” pill for lessons not yet visible to learners. */
  draftLessonIds?: ReadonlySet<number>;
};

export function CourseOverviewExperience({
  courseTitle,
  overviewBlurb,
  unlocked,
  outline,
  ...
  banner,
  draftLessonIds,
}: CourseOverviewExperienceProps) {
  const lessonIdsLength = outline.reduce((n, m) => n + m.lessons.length, 0);
  const estMin = estimateMinutesForLessons(lessonIdsLength);
                ...
                <CardContent>
                  <div className="grid gap-3 sm:grid-cols-2">
                    {mod.lessons.map((lesson, li) => {
                      ...
                      const href = unlocked
                        ? lessonHref(lesson.lessonKey)
                        : '/pricing?reason=subscription';
                      const showDraftPill =
                        Boolean(draftLessonIds?.has(lesson.id)) && unlocked;
